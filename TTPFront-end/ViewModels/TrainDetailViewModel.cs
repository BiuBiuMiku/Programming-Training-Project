...
public partial class TrainDetailViewModel : ObservableObject
{
    ...
    [ObservableProperty] private string _trainNo; [ObservableProperty] private string _date;
    [ObservableProperty] private string _fromStation = string.Empty; [ObservableProperty] private string _toStation = string.Empty;
    [ObservableProperty] private string _departureTime = string.Empty; [ObservableProperty] private string _arrivalTime = string.Empty; [ObservableProperty] private string _duration = string.Empty;
    ...
    [ObservableProperty] private bool _isLoading = true; [ObservableProperty] private string _errorMessage = string.Empty;
    ...
    public async Task LoadDetailAsync()
    {
        ...
        try
        {
            ...
            if (r.IsSuccessStatusCode) { var d = await r.Content.ReadFromJsonAsync<ApiResponse<TrainDetailInfo>>(); if (d?.Data != null) { var t = d.Data; FromStation = t.FromStation; ToStation = t.ToStation; DepartureTime = t.DepartureTime; ArrivalTime = t.ArrivalTime; Duration = t.Duration; Stops.Clear(); foreach (var s in t.Stops) Stops.Add(s); SeatTypes.Clear(); foreach (var s in t.SeatTypes) SeatTypes.Add(s); } }
            else { var e = await r.Content.ReadFromJsonAsync<ApiResponse<object>>(); ErrorMessage = e?.Message ?? "加载失败"; }
        }
